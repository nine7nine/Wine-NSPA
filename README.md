...
| [NTSync PI Kernel](https://nine7nine.github.io/Wine-NSPA/ntsync-pi-driver.gen.html) | Wine-NSPA's ntsync kernel overlay: PI primitives, channel transport, aggregate-wait, `CHANNEL_TRY_RECV2`, and the later slab / lockless-scan hardening work. Production module srcversion `F1A9EA24E257A35BB21341D`. |
...
The 2026-05-04 ntsync follow-ons advanced the production module from `10124FB81FDC76797EF1F91` to `F1A9EA24E257A35BB21341D`. The shipped additions are the receive-snapshot fix for channel pops, dedicated slab caches for the hot ntsync allocation classes, and the lockless `SEND_PI` target scan, plus the small NULL-guard follow-up in the slab conversion audit. Cumulative validation: ~14M ntsync ops KASAN-clean on debug kernel, full RT-suite v7 pass on the production kernel.
...
- **NTSync kernel overlay + userspace sync**: PI primitives, direct wait/signal path, aggregate-wait, and `TRY_RECV2` are in production; current module `F1A9EA24E257A35BB21341D` with the receive-snapshot fix, dedicated slab caches, and the lockless `SEND_PI` target scan
