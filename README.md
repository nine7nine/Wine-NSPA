...
| [NTSync PI Driver](https://nine7nine.github.io/Wine-NSPA/ntsync-pi-driver.gen.html) | Kernel-side ntsync overlay: PI primitives (1003), channel transport (1004/1005/1007/1009/1012), RT alloc-hoist (1006), deferred `EVENT_SET_PI` (1008), aggregate-wait (1010), `CHANNEL_TRY_RECV2` (1011), dedicated `kmem_cache`s (1013), and `SEND_PI` lockless target scan (1014/1014a). Production module srcversion `F1A9EA24E257A35BB21341D`. |
| [NTSync Userspace Integration](https://nine7nine.github.io/Wine-NSPA/ntsync-userspace.gen.html) | Wine ntdll handle-to-fd cache, server-owned vs. client-created anonymous sync handle paths, `linux_wait_objs` / `linux_set_event_obj_pi`, dispatcher channel ioctl wrappers, and PE-side wait coverage. |
...
**WIP.** The 11.x tree's current published full-suite boundary remains Layer 1 native ntsync 3 PASS / 0 FAIL plus Layer 2 PE matrix 24 PASS / 0 FAIL / 0 TIMEOUT. The newer shipped 2026-05-02 / 2026-05-03 / 2026-05-04 carries are documented as targeted follow-on validation rather than a synthetic v9.

The 2026-05-04 ntsync follow-ons advance the production module from `10124FB81FDC76797EF1F91` (post-1011) to `F1A9EA24E257A35BB21341D` (post-1014a). 1012 closes a KASAN-caught `CHANNEL_RECV` / `RECV2` cross-thread slab UAF by snapshotting popped-entry fields under `obj_lock` before unlock; 1013 moves the three hot ntsync allocation classes (`ntsync_event_pi`, `ntsync_channel_entry`, `ntsync_pi_owner`) into dedicated `kmem_cache`s with `SLAB_HWCACHE_ALIGN` for cacheline isolation and `SLAB_FREELIST_HARDENED` coverage; 1014 adds a `list_empty_careful` fast-path on `SEND_PI`'s boost-target scan (skips `wq->lock` round-trip on the common empty-queue case); 1014a NULL-guards the `kmem_cache_free` site missed in the 1013 conversion audit. Cumulative validation: ~14M ntsync ops KASAN-clean on debug kernel, full RT-suite v7 pass on prod kernel.
...
- **NTSync + in-process sync**: PI primitives, direct wait/signal path, aggregate-wait, and `TRY_RECV2` are in production; current module `F1A9EA24E257A35BB21341D` (post-1014a; carries 1003-1014 + the channel field-snapshot UAF fix, dedicated `kmem_cache`s, and the SEND_PI lockless target scan)
