...
Sized at 64 slots × 128 bytes ≈ 8 KB. Current header layout
...
The 2026-05-16 layout change is load-bearing, not cosmetic. The hot producer
`head` CAS and the consumer's `tail` advance no longer bounce the same
cacheline on every same-process message transfer. `overflow` also gets its own
aligned line, while the remaining control fields stay co-located on a third
cacheline because they are not the primary producer/consumer ping-pong pair.
The timer and redraw rings follow the same cacheline discipline for the same
reason.
...
<svg width="100%" viewBox="0 0 920 410" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="460" y="24" class="rl-label-accent" text-anchor="middle">nspa_queue_bypass_shm_t (memfd, cacheline-isolated ring headers)</text>

  <rect x="20" y="35" width="880" height="355" rx="10" fill="none" stroke="#6b7398" stroke-width="1" stroke-dasharray="5,3"/>
  ...
  <rect x="60" y="85" width="180" height="38" class="rl-box"/>
  <text x="150" y="102" class="rl-label-cyan" text-anchor="middle">cacheline 0: `head`</text>
  <text x="150" y="115" class="rl-label-sm" text-anchor="middle">producer CAS only</text>

  <rect x="260" y="85" width="180" height="38" class="rl-box"/>
  <text x="350" y="102" class="rl-label-cyan" text-anchor="middle">cacheline 1: `tail`</text>
  <text x="350" y="115" class="rl-label-sm" text-anchor="middle">consumer advance only</text>

  <rect x="460" y="85" width="400" height="38" class="rl-box"/>
  <text x="660" y="102" class="rl-label-cyan" text-anchor="middle">cacheline 2: `overflow`, `active`, `pending_*`, `next_post_seq`, `change_*`</text>
  <text x="660" y="115" class="rl-label-sm" text-anchor="middle">control fields and counters that are not the head/tail ping-pong pair</text>

  <text x="70" y="143" class="rl-label-yellow">slots[0..63]</text>
  <text x="70" y="155" class="rl-label-muted">`slots[]` begins on its own 64-byte boundary; each slot is `nspa_msg_slot_t` (128 B)</text>
...
The timer and redraw rings are simpler SPSC variants, but they now follow the
same cacheline split on `head` / `tail` / `overflow`: keep the hot writer and
hot reader indices from sharing a line, then align `slots[]` separately so the
payload array starts cleanly.
...
<svg width="100%" viewBox="0 0 940 420" xmlns="http://www.w3.org/2000/svg">
  <style>
    ...
    .pc-line-b { stroke: #7aa2f7; stroke-width: 1.4; }
    .pc-line-g { stroke: #9ece6a; stroke-width: 1.4; }
    .pc-line-r { stroke: #f7768e; stroke-width: 1.4; }
  </style>
  ...
  <line x1="300" y1="130" x2="355" y2="130" class="pc-line-b"/>
  <line x1="585" y1="130" x2="640" y2="130" class="pc-line-r"/>
  ...
  <line x1="470" y1="194" x2="250" y2="254" class="pc-line-g"/>
  <line x1="470" y1="194" x2="690" y2="254" class="pc-line-r"/>
